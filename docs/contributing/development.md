...
When you have two clones of the repository (e.g. `ByteTensor` and `ByteTensor-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `BYTETENSOR_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`ByteTensor-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.bytetensor-dev-2`, `~/.bytetensor-config-dev-2`)
...
ByteTensor uses **electron-vite** for fast bundling:
