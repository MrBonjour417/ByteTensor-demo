...
<p align="center">
  <strong>💬 コミュニティ：</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-10.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/ByteTensor" target="_blank">Twitter</a>
</p>
...
[🤔 なぜ ByteTensor を選ぶのか？](#-なぜ-bytetensor-を-claude-cowork-より選ぶのか) ·
[🚀 クイックスタート](#-クイックスタート) ·
[💬 コミュニティ](#-コミュニティとサポート)
...
**ByteTensor は単なるチャットクライアントではありません。** これは Cowork プラットフォームであり、AI Agent があなたのコンピューター上であなたと共に働きます——ファイルを読み、コードを書き、ウェブを閲覧し、タスクを自動化します。Agent のすべての動作を見ることができ、常にあなたがコントロールしています。

|                                       | 従来の AI チャットクライアント | **ByteTensor (Cowork)**                                                                                              |
| :------------------------------------ | :----------------------------- | :------------------------------------------------------------------------------------------------------------------- |
...
ByteTensor には完全な AI Agent エンジンが内蔵されています。CLI Agent を個別にインストールする必要があるツールとは異なり、**ByteTensor はインストールした瞬間から動作します**。
...
_ByteTensor には専用の Morph PPT アシスタントが内蔵されています。静的なスライドを作るだけでなく、内容を“語れる”動的プレゼンに変えます。テーマ/アウトライン（または参考スタイル）を入力するだけで、連続感のある Morph プレゼンを生成できます。基盤は [OfficeCLI](https://github.com/iOfficeAI/OfficeCli#) で動作します。_

- **普通の PPT 生成とは違う** — Morph によるストーリー表現とスライド間の連続的な動きに特化
- **ワークフローを内蔵** — 企画、生成、品質チェック、反復を一体化
- **スタイル合わせに対応** — 参考画像/参考スタイルで見た目を素早く調整
- **生成してすぐ確認** — `.ppt` / `.pptx` を ByteTensor 上でそのままプレビュー
...
すでに Claude Code、Codex、Hermes Agent、または OpenClaw を使用している場合、ByteTensor は自動的に検出し、それらすべてと Cowork できます——内蔵 Agent と一緒に。

**サポートされている Agent：** 内蔵 Agent（ゼロ設定） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • ByteTensor CLI（ByteTensor に同梱される Rust バックエンドサービス） • Snow CLI • Hermes Agent • Cursor Agent など
...
- **並列マルチエージェント実行** — Leader がタスクをサブタスクに分解して並行実行する Teammate Agent に委任；各 Teammate は ACP（Agent Communication Protocol、ByteTensor のマルチエージェント調整層）、Gemini、または ByteTensor CLI を通じて独自のモデルを使用
- **Leader によるオーケストレーション** — Leader が割り当て、追跡、集約；対応バックエンドは Claude Code、Codex、Hermes Agent、Gemini、Snow CLI、ByteTensor CLI
...
- **対応バックエンド** — Claude Code、Codex、Gemini、Snow CLI、ByteTensor CLI；`mcpCapabilities.stdio` を持つ他の ACP バックエンドも自動対応
...
他の AI アプリは API キー付きのチャットボックスを提供します。**ByteTensor は完全な Cowork Agent を提供します。**

| あなたの API キー              | 得られる機能                |
| :----------------------------- | :-------------------------- |
| Gemini API キー                | Gemini 駆動の Cowork Agent  |
| OpenAI API キー                | GPT 駆動の Cowork Agent     |
| Anthropic API キー             | Claude 駆動の Cowork Agent  |
| Ollama / LM Studio（ローカル） | ローカルモデル Cowork Agent |
| NewAPI ゲートウェイ            | 20+ モデルへの統一アクセス  |

どのモデルで動かしても、Agent の機能は同じ——ファイル読み書き、ウェブ検索、画像生成、ツール使用。ByteTensor はクラウドサービスとローカルデプロイメントを含む **30+ の AI プラットフォーム**をサポートしています。
...
<details>
<summary><strong>🔍 30+ サポートプラットフォームをすべて表示 ▶️</strong></summary>

<br>

**包括的なプラットフォームサポート：**

- **公式プラットフォーム** — Gemini、Gemini (Vertex AI)、Anthropic (Claude)、OpenAI
- **クラウドプロバイダー** — AWS Bedrock、New API（統一 AI モデルゲートウェイ）
- **中国プラットフォーム** — Dashscope (Qwen)、Dashscope コーディングプラン、Zhipu、Moonshot (Kimi)、Qianfan (Baidu)、Hunyuan (Tencent)、Lingyi、ModelScope、InfiniAI、Ctyun、StepFun、SiliconFlow-CN、PPIO
...
ByteTensor は [NewAPI](https://github.com/QuantumNous/new-api) ゲートウェイサービスもサポートしています——様々な LLM を集約・配布する統一 AI モデルハブです。同じインターフェースで異なるモデルを柔軟に切り替えて、様々なタスク要件に対応できます。
...
- **三層スキル** — 内蔵スキル（ByteTensor に同梱）、カスタムスキル（ユーザー定義）、Extension スキル（サードパーティ Extension から提供）；スキルインジケーターで会話ごとに有効化/無効化
...
<details>
<summary><strong>🔍 アシスタント詳細とカスタムスキルを表示 ▶️</strong></summary>

<br>

ByteTensor には、事前定義された機能を持つ **21 個のプロフェッショナルアシスタント**が含まれており、カスタムスキルで拡張可能です：
...
- **📋 Word Form Creator** — 構造化された Word フォーム／契約テンプレートの生成
...
- **📣 ソーシャル求人公開** — 求人投稿と公開
- **🦞 moltbook** — ゼロデプロイ AI Agent ソーシャルネットワーキング
...
**カスタムスキル**：`skills/` ディレクトリにスキルを作成し、アシスタントのスキルを有効化/無効化して AI 機能を拡張します。スキルは三つのソースから提供されます：内蔵（ByteTensor に同梱）、カスタム（ユーザー定義）、Extension（Extension SDK 経由で提供）。内蔵スキルには `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` などが含まれます。
...
_あなたの 24/7 AI アシスタント — あらゆるデバイス、どこからでも ByteTensor にアクセス。_
...
> **設定：** ByteTensor 設定 → WebUI 設定 → Channel、Bot Token を設定します。
...
<details>
<summary><strong>🔍 スケジュールタスクの詳細を表示 ▶️</strong></summary>
...
- **スリープ防止** — タスク実行中に ByteTensor がシステムのスリープを自動防止し、復帰後のトリガー漏れを検出
...
### **プレビューパネル — AI 生成結果を即座に表示**

_10+ フォーマット：PDF、Word、Excel、PPT、コード、Markdown、画像、HTML、Diff — アプリを切り替えることなくすべてを表示。_

- **即座のプレビュー** — Agent がファイルを生成した後、アプリを切り替えることなく結果を即座に表示
- **リアルタイム追跡 + 編集可能** — ファイル変更を自動追跡；Markdown、コード、HTML のライブ編集をサポート
- **マルチタブサポート** — 複数のファイルを同時に開き、それぞれ独自のタブで
...
<details>
<summary><strong>🔍 完全なフォーマットリストを表示 ▶️</strong></summary>

<br>

**サポートされているプレビューフォーマット：**

- **ドキュメント** — PDF、Word (`.doc`, `.docx`, `.odt`)、Excel (`.xls`, `.xlsx`, `.ods`, `.csv`)、PowerPoint (`.ppt`, `.pptx`, `.odp`)
- **コード** — JavaScript、TypeScript、Python、Java、Go、Rust、C/C++、CSS、JSON、XML、YAML、Shell スクリプト、および 30+ のプログラミング言語
- **マークアップ** — Markdown (`.md`, `.markdown`)、HTML (`.html`, `.htm`)
- **画像** — PNG、JPG、JPEG、GIF、SVG、WebP、BMP、ICO、TIFF、AVIF
- **その他** — Diff ファイル (`.diff`, `.patch`)

</details>

---

### **スマートファイル管理 — 自動化されたファイル操作**
...
- **自動整理** — コンテンツをインテリジェントに識別し、自動分類してフォルダを整理
- **効率的なバッチ** — ワンクリックでリネーム、ファイルマージ、面倒な手動タスクに別れを告げる
- **自動化実行** — AI Agent が独立してファイル操作を実行し、ファイルを読み書きし、タスクを自動的に完了

**使用例：**

- ファイルタイプ別に乱雑なダウンロードフォルダを整理
- 意味のある名前で写真をバッチリネーム
- 複数のドキュメントを 1 つにマージ
- コンテンツ別にファイルを自動分類
...
<details>
<summary><strong>🔍 Excel 処理機能を表示 ▶️</strong></summary>

<br>

- **スマート分析** — AI がデータパターンを分析し、インサイトを生成
- **自動フォーマット** — プロフェッショナルなスタイリングで Excel レポートを自動的に美化
- **データ変換** — 自然言語コマンドでデータを変換、マージ、再構築
- **レポート生成** — 生データから包括的なレポートを作成

**使用例：**

- 売上データを分析し、月次レポートを生成
- 乱雑な Excel ファイルをクリーンアップしてフォーマット
- 複数のスプレッドシートをインテリジェントにマージ
- データ可視化とチャートを作成
...
<details>
<summary><strong>🔍 画像生成機能を表示 ▶️</strong></summary>

<br>

- **テキストから画像** — 自然言語の説明から画像を生成
- **画像編集** — 既存の画像を変更および強化
- **画像認識** — 画像コンテンツを分析および記述
- **バッチ処理** — 一度に複数の画像を生成
...
<details>
<summary><strong>🔍 ドキュメント生成機能を表示 ▶️</strong></summary>

<br>

- **PPTX ジェネレーター** — アウトラインまたはトピックからプロフェッショナルなプレゼンテーションを作成
- **Word ドキュメント** — 適切な構造を持つフォーマット済み Word ドキュメントを生成
- **Markdown ファイル** — ドキュメント用の Markdown ドキュメントを作成およびフォーマット
- **PDF 変換** — 様々なドキュメントフォーマット間で変換
...
- ✅ **並列実行** — 干渉なく複数のタスクを同時実行
- ✅ **スマート管理** — 視覚的インジケーターで会話間を簡単に切り替え

---

## 🤔 なぜ ByteTensor を Claude Cowork より選ぶのか？
...
ByteTensor は**無料でオープンソースの Multi-AI Agent デスクトップ**です。macOS でのみ動作し Claude にロックされている Claude Cowork と比較して、ByteTensor は全モデル、クロスプラットフォームの強化版です。

| 次元             | Claude Cowork    | ByteTensor                                            |
| :--------------- | :--------------- | :---------------------------------------------------- |
| OS               | macOS のみ       | macOS / Windows / Linux                               |
| モデルサポート   | Claude のみ      | Gemini、Claude、DeepSeek、OpenAI、Ollama、...         |
| インタラクション | デスクトップ GUI | デスクトップ GUI + WebUI + Telegram / Lark / DingTalk |
| 自動化           | 手動のみ         | Cron スケジュールタスク — 24/7 無人運転               |
| コスト           | $100/月          | 無料かつオープンソース                                |

深い AI オフィスシナリオサポート：

- **ファイル管理**：ローカルフォルダをインテリジェントに整理し、ワンクリックでバッチリネーム。
- **データ処理**：Excel レポートを深く分析し、自動的に美化。
- **ドキュメント生成**：PPT、Word、Markdown ドキュメントを自動的に作成およびフォーマット。
- **即座のプレビュー**：内蔵 10+ フォーマットプレビューパネル、AI コラボレーション結果が即座に表示可能。
...
A: <strong>いいえ。</strong> ByteTensor にはインストール後すぐに動作する内蔵 AI Agent があります。任意の API キーを入力するだけです。Claude Code や Gemini CLI などの CLI ツールもインストールしている場合、ByteTensor は自動的に検出して統合し、さらに多くの機能を提供します。
</details>

<details>
<summary><strong>Q: ByteTensor で何ができますか？</strong></summary>
A: ByteTensor はあなたの<strong>プライベート Cowork ワークスペース</strong>です。内蔵 Agent はフォルダをバッチ整理し、Excel データを処理し、ドキュメントを生成し、ウェブを検索し、画像を生成できます。マルチ Agent モードでは、同じインターフェースを通じて Claude Code、Codex、その他の強力な CLI Agent も活用できます。
</details>

<details>
<summary><strong>Q: 無料ですか？</strong></summary>
A: ByteTensor は完全に無料でオープンソースです。利用するプロバイダーの API 使用料のみがかかり、お好みのプロバイダーの API キーを使用できます。
</details>

<details>
<summary><strong>Q: データは安全ですか？</strong></summary>
A: すべてのデータはローカルの SQLite データベースに保存されます。サーバーにアップロードされることはありません。
</details>
...
<p align="center">
  <em>WorldofAI (20 万登録者)</em> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <em>Julian Goldie SEO (38.4 万登録者)</em>
</p>

### コミュニティ記事

- [オープンソース無料 Cowork、全モデル統合 + 自律ファイル操作](https://mp.weixin.qq.com/s/F3f-CCsVPaK3lK00jXhOOg) — オープンソース AI プロジェクトの実現
...
> **ByteTensor について動画を作成しましたか？** [X でお知らせください](https://x.com/ByteTensor)、ここで紹介します！
...
<details>
<summary><strong>📖 完全な使用ガイドを展開して表示</strong></summary>
...
- [WeChat グループ](../../resources/wx-10.png) — 中国語コミュニティ
