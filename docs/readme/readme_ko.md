...
<p align="center">
  <strong>💬 커뮤니티:</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-10.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/ByteTensor" target="_blank">Twitter</a>
</p>
...
[✨ Cowork 실연](#-cowork-실연) ·
[🤔 왜 ByteTensor를 선택해야 하나요?](#-왜-bytetensor를-claude-cowork보다-선택해야-하나요) ·
[🚀 빠른 시작](#-빠른-시작) ·
[💬 커뮤니티](#-커뮤니티-및-지원)
...
**ByteTensor는 단순한 채팅 클라이언트가 아닙니다.** 컴퓨터에서 AI Agent가 함께 작업하는 Cowork 플랫폼입니다——파일 읽기, 코드 작성, 웹 브라우징, 작업 자동화. Agent의 모든 동작을 볼 수 있으며, 항상 제어권은 여러분에게 있습니다.

|                                   | 기존 AI 채팅 클라이언트 | **ByteTensor (Cowork)**                                                                                        |
| :-------------------------------- | :---------------------- | :------------------------------------------------------------------------------------------------------------- |
...
ByteTensor는 완전한 AI Agent 엔진을 내장하고 있습니다. CLI Agent를 별도로 설치해야 하는 도구와 달리, **ByteTensor는 설치하는 순간 바로 작동합니다**.
...
다른 AI 앱은 API 키가 있는 채팅박스를 제공합니다. **ByteTensor는 완전한 Cowork Agent를 제공합니다.**
...
어떤 모델로 구동하든 Agent의 기능은 동일합니다——파일 읽기/쓰기, 웹 검색, 이미지 생성, 도구 사용. ByteTensor는 클라우드 서비스와 로컬 배포를 포함한 **30+ AI 플랫폼**을 지원합니다.
...
ByteTensor는 [NewAPI](https://github.com/QuantumNous/new-api) 게이트웨이 서비스도 지원합니다——다양한 LLM을 집계하고 배포하는 통합 AI 모델 허브입니다. 같은 인터페이스에서 다양한 모델을 유연하게 전환하여 다양한 작업 요구사항을 충족할 수 있습니다.
...
- **3단계 스킬** — 내장 스킬(ByteTensor 기본 제공), 커스텀 스킬(사용자 정의), Extension 스킬(서드파티 Extension 제공); 스킬 인디케이터로 대화별 활성화/비활성화
...
<details>
<summary><strong>🔍 어시스턴트 상세 및 커스텀 스킬 보기 ▶️</strong></summary>

<br>

ByteTensor는 사전 정의된 기능을 가진 **21개의 전문 어시스턴트**를 포함하며, 커스텀 스킬을 통해 확장 가능합니다:
...
- **🧭 HUMAN 3.0 코치** — 개인 개발 코치
- **📣 소셜 채용 게시** — 채용 게시 및 발행
- **🦞 moltbook** — 제로 배포 AI Agent 소셜 네트워킹
- **📈 Beautiful Mermaid** — 플로우차트, 시퀀스 다이어그램 등
- **🔧 OpenClaw 설정** — OpenClaw 통합을 위한 설정 및 구성 어시스턴트
- **📖 스토리 롤플레이** — 캐릭터 카드와 세계 정보를 갖춘 몰입형 스토리 롤플레이(SillyTavern 호환)

**커스텀 스킬**: `skills/` 디렉토리에 스킬을 만들고, 어시스턴트의 스킬을 활성화/비활성화하여 AI 기능을 확장합니다. 스킬은 세 가지 소스에서 제공됩니다: 내장(ByteTensor 기본 제공), 커스텀(사용자 정의), Extension(Extension SDK를 통해 제공). 내장 스킬에는 `pptx`, `docx`, `pdf`, `xlsx`, `mermaid` 등이 포함됩니다.
...
_여러분의 24/7 AI 어시스턴트 — 어떤 기기, 어디서나 ByteTensor에 접근하세요._
...
> **설정:** ByteTensor 설정 → WebUI 설정 → Channel, Bot Token을 구성하세요.
...
- **자연어** — 채팅하듯 Agent에게 무엇을 할지 말하세요
- **세 가지 스케줄링 모드** — 표준 cron 표현식(타임존 지원), 고정 간격(N분/시간마다), 또는 일회성 트리거
- **AI가 생성한 작업** — Agent가 대화 중 자율적으로 스케줄 작업 생성 가능
...
<details>
<summary><strong>🔍 스케줄 작업 상세 보기 ▶️</strong></summary>

<br>

**스케줄링 모드:**

- `Cron 표현식` — 타임존 지원 표준 5필드 cron(예: `0 9 * * 1`, `Asia/Shanghai`)
- `N분/시간마다` — 고정 간격, 예: 30분마다 실행
- `일회성` — 지정된 날짜/시간에 한 번만 트리거, 이후 자동 비활성화

**실행 모드:**

- `기존 대화 계속` — 바인딩된 대화에 이어서 추가하여 AI가 전체 컨텍스트 기록 유지
- `매번 새 대화 생성` — 트리거마다 새 세션 열기, 독립적인 주기 보고서에 이상적
...
- **절전 방지** — ByteTensor가 작업 활성화 중 시스템 절전 자동 방지, 절전 해제 후 누락된 트리거 감지
...
### **미리보기 패널 — AI 생성 결과 즉시 보기**

_10+ 형식: PDF, Word, Excel, PPT, 코드, Markdown, 이미지, HTML, Diff — 앱을 전환하지 않고 모든 것을 볼 수 있습니다._

- **즉시 미리보기** — Agent가 파일을 생성한 후 앱을 전환하지 않고 즉시 결과를 볼 수 있습니다
...
- **문서** — PDF, Word (`.doc`, `.docx`, `.odt`), Excel (`.xls`, `.xlsx`, `.ods`, `.csv`), PowerPoint (`.ppt`, `.pptx`, `.odp`)
- **코드** — JavaScript, TypeScript, Python, Java, Go, Rust, C/C++, CSS, JSON, XML, YAML, Shell 스크립트 및 30+ 프로그래밍 언어
- **마크업** — Markdown (`.md`, `.markdown`), HTML (`.html`, `.htm`)
- **이미지** — PNG, JPG, JPEG, GIF, SVG, WebP, BMP, ICO, TIFF, AVIF
...
- **자동 정리** — 콘텐츠를 지능적으로 식별하고 자동 분류하여 폴더를 깔끔하게 유지
- **효율적인 일괄 처리** — 원클릭 이름 변경, 파일 병합, 지루한 수동 작업에 작별 인사
- **자동화 실행** — AI Agent가 독립적으로 파일 작업을 실행하고, 파일을 읽고 쓰며, 작업을 자동으로 완료합니다

**사용 사례:**

- 파일 유형별로 지저분한 다운로드 폴더 정리
- 의미 있는 이름으로 사진 일괄 이름 변경
- 여러 문서를 하나로 병합
- 콘텐츠별로 파일 자동 분류
...
<details>
<summary><strong>🔍 Excel 처리 기능 보기 ▶️</strong></summary>

<br>

- **스마트 분석** — AI가 데이터 패턴을 분석하고 인사이트를 생성합니다
- **자동 포맷팅** — 전문적인 스타일링으로 Excel 보고서를 자동으로 아름답게 만듭니다
- **데이터 변환** — 자연어 명령으로 데이터를 변환, 병합 및 재구성합니다
- **보고서 생성** — 원시 데이터에서 포괄적인 보고서를 만듭니다

**사용 사례:**

- 판매 데이터를 분석하고 월간 보고서 생성
- 지저분한 Excel 파일 정리 및 포맷팅
- 여러 스프레드시트를 지능적으로 병합
- 데이터 시각화 및 차트 생성
...
- **텍스트-이미지** — 자연어 설명에서 이미지 생성
- **이미지 편집** — 기존 이미지 수정 및 향상
- **이미지 인식** — 이미지 콘텐츠 분석 및 설명
- **일괄 처리** — 한 번에 여러 이미지 생성
...
<details>
<summary><strong>🔍 문서 생성 기능 보기 ▶️</strong></summary>

<br>

- **PPTX 생성기** — 개요 또는 주제에서 전문 프레젠테이션 만들기
- **Word 문서** — 적절한 구조를 가진 포맷된 Word 문서 생성
- **Markdown 파일** — 문서화를 위한 Markdown 문서 생성 및 포맷팅
- **PDF 변환** — 다양한 문서 형식 간 변환
...
## 🤔 왜 ByteTensor를 Claude Cowork보다 선택해야 하나요?
...
ByteTensor는 **무료 오픈소스 Multi-AI Agent 데스크톱**입니다. macOS에서만 실행되고 Claude에 고정된 Claude Cowork과 비교하여, ByteTensor는 전체 모델, 크로스 플랫폼 강화 버전입니다.

| 차원      | Claude Cowork | ByteTensor                                        |
| :-------- | :------------ | :------------------------------------------------ |
...
- **파일 관리**: 로컬 폴더를 지능적으로 정리하고 원클릭으로 일괄 이름 변경.
- **데이터 처리**: Excel 보고서를 깊이 분석하고 자동으로 아름답게 만듭니다.
- **문서 생성**: PPT, Word, Markdown 문서를 자동으로 작성하고 포맷팅합니다.
- **즉시 미리보기**: 내장 10+ 형식 미리보기 패널, AI 협업 결과가 즉시 표시됩니다.
...
A: <strong>아니요.</strong> ByteTensor는 설치 후 즉시 작동하는 내장 AI Agent가 있습니다. API 키를 입력하기만 하면 됩니다. Claude Code나 Gemini CLI와 같은 CLI 도구도 설치되어 있다면, ByteTensor가 자동으로 감지하고 통합하여 더 많은 기능을 제공합니다.
</details>

<details>
<summary><strong>Q: ByteTensor로 무엇을 할 수 있나요?</strong></summary>
A: ByteTensor는 여러분의 <strong>프라이빗 Cowork 작업 공간</strong>입니다. 내장 Agent는 폴더를 일괄 정리하고, Excel 데이터를 처리하고, 문서를 생성하고, 웹을 검색하고, 이미지를 생성할 수 있습니다. 멀티 Agent 모드에서는 같은 인터페이스를 통해 Claude Code, Codex 및 기타 강력한 CLI Agent도 활용할 수 있습니다.
</details>

<details>
<summary><strong>Q: 무료인가요?</strong></summary>
A: ByteTensor는 완전히 무료이며 오픈소스입니다. 선택한 제공업체의 API 사용량에 대해서만 비용을 지불하며, 선호하는 제공업체의 API 키를 사용할 수 있습니다.
</details>

<details>
<summary><strong>Q: 내 데이터는 안전한가요?</strong></summary>
A: 모든 데이터는 로컬 SQLite 데이터베이스에 저장됩니다. 어떤 서버에도 업로드되지 않습니다.
</details>
...
<p align="center">
  <em>WorldofAI (20만 구독자)</em> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <em>Julian Goldie SEO (38.4만 구독자)</em>
</p>
...
> **ByteTensor에 대한 동영상을 만들었나요?** [X에서 알려주세요](https://x.com/ByteTensor), 여기에 소개하겠습니다!
...
위 버튼을 클릭하여 Releases 페이지로 이동한 후 플랫폼에 맞는 설치 파일을 다운로드하세요（macOS / Windows / Linux）。
