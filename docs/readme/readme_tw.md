...
<p align="center">
  <strong>💬 社群：</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-10.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/ByteTensor" target="_blank">Twitter</a>
</p>
...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 為什麼選擇 ByteTensor？](#-為什麼選擇-bytetensor-而非-claude-cowork) ·
[🚀 快速開始](#-快速開始) ·
[💬 社群](#-社群與支援)
...
**ByteTensor 不只是個聊天工具。** 它是一個真正的 Cowork 平台，AI Agent 就像您的得力助手，在電腦上幫您處理各種任務——讀檔案、寫程式碼、查資料、自動化工作流。Agent 的一舉一動都在您的掌控之中，透明可見。

|                       | 傳統 AI 聊天客戶端 | **ByteTensor (Cowork)**                                                                                |
| :-------------------- | :----------------- | :----------------------------------------------------------------------------------------------------- |
...
ByteTensor 自帶完整的 AI Agent 引擎。不像其他工具需要您手動安裝各種 CLI 工具，**ByteTensor 裝好就能用，開箱即用**。
...
想把文件/表格直接交給 Agent？ByteTensor 內建 **[OfficeCLI](https://github.com/iOfficeAI/OfficeCli)**，讓 PPT（Morph 轉場）、Word（`.docx`）與 Excel（`.xlsx/.xlsm/.csv`）從需求到可交付結果更高效、更穩定。
...
如果您已經在用 Claude Code、Codex 或 Qwen Code，ByteTensor 會自動發現它們，讓您同時和這些 Agent 一起 Cowork——當然，還有內建 Agent。

**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • ByteTensor CLI（隨附於 ByteTensor 的 Rust 後端服務） • Snow CLI • Hermes Agent • Cursor Agent 等
...
- **多 Agent 並行執行** — Leader 將任務分解為子任務並委派給並行運行的 Teammate Agent；每個 Teammate 透過 ACP（Agent Communication Protocol，ByteTensor 的多 Agent 協調層）、Gemini 或 ByteTensor CLI 使用獨立模型
- **Leader 統籌編排** — Leader 分配、追蹤並彙總結果；支援的後端包括 Claude Code、Codex、Hermes Agent、Gemini、Snow CLI 和 ByteTensor CLI
...
- **支援的後端** — Claude Code、Codex、Gemini、Snow CLI、ByteTensor CLI；其他具備 `mcpCapabilities.stdio` 的 ACP 後端自動支援
...
ByteTensor 還支援 [NewAPI](https://github.com/QuantumNous/new-api) 閘道服務 — 一個統一的 AI 模型中心，聚合和分發各種大語言模型。在同一個介面中靈活切換不同模型，滿足各種任務需求。
...
- **三層技能體系** — 內建技能（隨 ByteTensor 附帶）、自訂技能（您自己的）以及擴展技能（第三方擴展貢獻）；透過技能指示器按對話啟用/停用
...
ByteTensor 內建 **21 個專業助手**，每個都有獨特能力，還能透過自訂技能繼續擴展：
...
**自訂技能**：在 `skills/` 目錄下建立您的專屬技能，隨時為助手開啟或關閉，讓 AI 能力無限擴展。技能來源分三層：內建（隨 ByteTensor 附帶）、自訂（您自己的）以及擴展（透過擴展 SDK 貢獻）。內建技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。
...
> **設定：** ByteTensor 設定 → WebUI 設定 → Channel，配置 Bot Token。
...
- **防休眠** — ByteTensor 會自動阻止系統休眠，任務啟用期間偵測喚醒後的漏觸發
...
## 🤔 為什麼選擇 ByteTensor 而非 Claude Cowork？
...
ByteTensor 是一個**免費開源的 Multi-AI Agent 桌面應用**。相比只能在 macOS 上用、還只能綁定 Claude 的 Claude Cowork，ByteTensor 支援全模型、跨平台，是它的全面升級版。

| 維度     | Claude Cowork | ByteTensor                                             |
| :------- | :------------ | :----------------------------------------------------- |
...
答：<strong>完全不需要。</strong> ByteTensor 自帶 AI Agent，裝好就能用。輸入任意 API 金鑰即可開始。如果您已經裝了 Claude Code 或 Gemini CLI 這些 CLI 工具，ByteTensor 會自動發現並整合它們，功能更強大。
</details>

<details>
<summary><strong>問：我可以用 ByteTensor 做什麼？</strong></summary>
答：ByteTensor 就是您的<strong>私有 Cowork 工作空間</strong>。內建 Agent 可以幫您批次整理資料夾、處理 Excel 資料、產生文件、搜尋網路、產生圖像。透過多 Agent 模式，您還能在同一介面同時使用 Claude Code、Codex 和其他強大的 CLI Agent。
</details>

<details>
<summary><strong>問：它是免費的嗎？</strong></summary>
答：ByteTensor 完全免費且開源。您只需為所選服務商的 API 用量付費，可以使用任何您喜歡的服務商的 API 金鑰。
</details>
...
> **製作了關於 ByteTensor 的影片？** [在 X 上告訴我們](https://x.com/ByteTensor)，我們會在這裡展示！
...
<td>
  <a href="https://packycode.com" target="_blank">PackyCode</a> 是一家可靠高效的 API 中繼服務提供商，為 Claude Code、Codex、Gemini 等平台提供中繼服務。感謝 PackyCode 為支持 ByteTensor 用戶低成本使用，為我們的用戶提供專屬 <a href="https://www.packyapi.com/register?aff=bytetensor" target="_blank">9折優惠</a>，點擊此連結並在付款時使用優惠碼 <code>bytetensor</code> 可立減 10%。
</td>
</tr>
</table>
