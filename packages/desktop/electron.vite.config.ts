...
// Read the real ByteTensor version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@bytetensor/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@bytetensor/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@bytetensor/web-host'] }),
        ...(isDevelopment
          ? [
              {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another ByteTensor clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.BYTETENSOR_MULTI_INSTANCE': JSON.stringify(process.env.BYTETENSOR_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real ByteTensor version (root package.json) so renderer code
