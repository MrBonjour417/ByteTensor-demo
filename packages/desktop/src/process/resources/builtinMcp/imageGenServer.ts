...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.BYTETENSOR_IMG_PLATFORM;
  const base_url = process.env.BYTETENSOR_IMG_BASE_URL;
  const api_key = process.env.BYTETENSOR_IMG_API_KEY;
  const model = process.env.BYTETENSOR_IMG_MODEL;
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris, workspace_dir }) => {
      ...
      const proxy = process.env.BYTETENSOR_IMG_PROXY || undefined;
