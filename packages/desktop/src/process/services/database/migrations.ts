...
const migration_v8: IMigration = {
  ...
  up: (db) => {
    ...
    if (!columns.has('source')) {
      db.exec(`ALTER TABLE conversations ADD COLUMN source TEXT CHECK(source IN ('bytetensor', 'telegram'))`);
...
const migration_v11: IMigration = {
  ...
  up: (db) => {
    ...
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('bytetensor', 'telegram')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        source TEXT CHECK(source IS NULL OR source IN ('bytetensor', 'telegram')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
...
const migration_v12: IMigration = {
  ...
  up: (db) => {
    ...
    // Clean up any invalid source values before copying
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('bytetensor', 'telegram', 'lark')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('bytetensor', 'telegram', 'lark')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
      )`);
...
const migration_v14: IMigration = {
  ...
  up: (db) => {
    ...
    // so DROP TABLE will NOT trigger ON DELETE CASCADE on the messages table.
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('bytetensor', 'telegram', 'lark', 'dingtalk')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('bytetensor', 'telegram', 'lark', 'dingtalk')),
