...
const BUNDLED_BACKEND_DIR = 'bundled-bytetensorcore';
...
export function collectBackendInstallDiagnostics(
  ...
  const bundledDirPath = resourcesPath ? pathApi.join(resourcesPath, BUNDLED_BACKEND_DIR) : undefined;
  const runtimeDirPath =
    resourcesPath && runtimeKey ? pathApi.join(resourcesPath, BUNDLED_BACKEND_DIR, runtimeKey) : undefined;
  const binaryPath =
