...
// Override via BYTETENSOR_CDP_PORT env variable. Set to "0" to disable.
//
...
// Registry file: ~/.bytetensor-cdp-registry.json
// ---------------------------------------------------------------------------

export const DEFAULT_CDP_PORT = 9230;
export const CDP_PORT_RANGE_START = 9230;
export const CDP_PORT_RANGE_END = 9250;
const CDP_REGISTRY_FILE = path.join(os.homedir(), '.bytetensor-cdp-registry.json');
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another ByteTensor instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active ByteTensor instances, trying ${preferredPort}`
  );
  return preferredPort;
}
...
function resolveCdpPortFromEnv(): number | null | undefined {
  const envVal = process.env.BYTETENSOR_CDP_PORT;
