...
const STORAGE_PATH = {
  config: 'bytetensor-config.txt',
  chatMessage: 'bytetensor-chat-message.txt',
  chat: 'bytetensor-chat.txt',
  env: '.bytetensor-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[ByteTensor] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[ByteTensor] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[ByteTensor] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('bytetensor.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'bytetensor-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'bytetensor-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'bytetensor-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'bytetensor-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[ByteTensor] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/bytetensor-app/assets/builtin-assistants/` and
 * `crates/bytetensor-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[ByteTensor:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see bytetensor-assistant crate) and
  ...
  try {
    ...
    console.error('[ByteTensor] Failed to ensure assistant dirs:', error);
  }
