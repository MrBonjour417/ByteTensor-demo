...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'bytetensor');
};
 ...
 * Release builds use ~/.bytetensor; dev builds use ~/.bytetensor-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.bytetensor，Dev 模式使用 ~/.bytetensor-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'bytetensor');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.bytetensor'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.bytetensor-config; dev builds use ~/.bytetensor-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.bytetensor-config，Dev 模式使用 ~/.bytetensor-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.bytetensor-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[ByteTensor] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[ByteTensor] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[ByteTensor] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[ByteTensor] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
