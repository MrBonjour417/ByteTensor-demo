...
import { startWebHost, type WebHostHandle } from '@bytetensor/web-host';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.BYTETENSOR_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.BYTETENSOR_ALLOW_REMOTE || process.env.BYTETENSOR_REMOTE);
  const hostHint = process.env.BYTETENSOR_HOST?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.bytetensor[-dev] symlink on macOS to sidestep
