  ...
  useEffect(() => {
    ...
    const handleConsoleMessage = (event: Electron.ConsoleMessageEvent) => {
      try {
        ...
        if (event.message.includes('__BYTETENSOR_WEBVIEW_ZOOM__')) {
        ...
        if (event.message.includes('__BYTETENSOR_WEBVIEW_ZOOM_RESET__')) {
          setZoomFactor(1);
        }
      } catch {
        // Ignore parse errors
      ...
      if (isStarOfficeUrl(currentUrl)) {
        webviewEl
          .executeJavaScript(
            `
          (function() {
            if (window.__bytetensorZoomInjected) return true;
            window.__bytetensorZoomInjected = true;
            window.addEventListener('wheel', function(e) {
              if (!(e.ctrlKey || e.metaKey)) return;
              e.preventDefault();
              console.log('__BYTETENSOR_WEBVIEW_ZOOM__', JSON.stringify({ deltaY: e.deltaY }));
            }, { passive: false, capture: true });
            window.addEventListener('keydown', function(e) {
              if (!(e.ctrlKey || e.metaKey)) return;
              if (e.key === '0') {
                e.preventDefault();
                console.log('__BYTETENSOR_WEBVIEW_ZOOM_RESET__');
              }
            }, { capture: true });
            return true;
          })();
