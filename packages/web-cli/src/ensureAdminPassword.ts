 ...
 *
 * On first tarball launch, ByteTensorCore's SQLite `users` table holds the
...
export type EnsureAdminPasswordOptions = {
  /** 127.0.0.1 port where ByteTensorCore listens (from WebHostHandle.backendPort). */
  backendPort: number;
   ...
   * by launch context — packaged tarball = `bytetensor-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'bytetensor-web resetpass';
  ...
  try {
    ...
    deps.warn(
      `[bytetensor-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`
    );
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[bytetensor-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[bytetensor-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[bytetensor-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[bytetensor-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[bytetensor-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[bytetensor-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
