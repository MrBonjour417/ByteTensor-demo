import { startWebHost, startStaticServer } from '@bytetensor/web-host';
import type { WebHostHandle, StaticServerHandle } from '@bytetensor/web-host';
...
function resolveCliRoot(): string {
  // Heuristic: if the executable path ends in "bytetensor-web" or "bytetensor-web.exe",
  // treat it as the packaged single-file binary and return its directory.
  const exe = process.execPath;
  const exeName = path.basename(exe).toLowerCase();
  if (exeName === 'bytetensor-web' || exeName === 'bytetensor-web.exe') {
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.BYTETENSOR_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.bytetensor-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.BYTETENSOR_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.BYTETENSOR_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.BYTETENSOR_ALLOW_REMOTE ?? process.env.BYTETENSOR_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[bytetensor-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[bytetensor-web] version    : ${version}`);
  console.log(`[bytetensor-web] data dir   : ${dataDir}`);
  console.log(`[bytetensor-web] log dir    : ${logDir}`);
  console.log(`[bytetensor-web] static dir : ${staticDir}`);
  console.log(`[bytetensor-web] backend bin: ${backendBin}`);
  console.log(`[bytetensor-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.log('');
    console.log('ByteTensor WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[bytetensor-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[bytetensor-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('ByteTensor WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[bytetensor-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[bytetensor-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `bytetensor-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[bytetensor-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set BYTETENSOR_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[bytetensor-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[bytetensor-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[bytetensor-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[bytetensor-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[bytetensor-web] username: ${username}`);
    console.log(`[bytetensor-web] new password: ${newPassword}`);
    console.log('[bytetensor-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: bytetensor-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.bytetensor-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.bytetensor-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  BYTETENSOR_PORT, BYTETENSOR_ALLOW_REMOTE, BYTETENSOR_DATA_DIR, BYTETENSOR_LOG_DIR,
  BYTETENSOR_BACKEND_BIN, BYTETENSOR_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: bytetensor-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[bytetensor-web] fatal:', err.message);
