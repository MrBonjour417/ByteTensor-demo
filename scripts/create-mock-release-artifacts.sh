...
# Windows x64
touch "$ARTIFACTS_DIR/windows-build-x64/ByteTensor-1.0.0-win-x64.exe"
cat > "$ARTIFACTS_DIR/windows-build-x64/latest.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0-win-x64.exe
    sha512: fake-sha512-x64
    size: 100000
path: ByteTensor-1.0.0-win-x64.exe
sha512: fake-sha512-x64
releaseDate: '2025-01-01'
EOF

# Windows arm64
touch "$ARTIFACTS_DIR/windows-build-arm64/ByteTensor-1.0.0-win-arm64.exe"
cat > "$ARTIFACTS_DIR/windows-build-arm64/latest.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0-win-arm64.exe
    sha512: fake-sha512-arm64
    size: 100000
path: ByteTensor-1.0.0-win-arm64.exe
sha512: fake-sha512-arm64
releaseDate: '2025-01-01'
EOF

# macOS x64
touch "$ARTIFACTS_DIR/macos-build-x64/ByteTensor-1.0.0-mac-x64.dmg"
touch "$ARTIFACTS_DIR/macos-build-x64/ByteTensor-1.0.0-mac-x64.zip"
cat > "$ARTIFACTS_DIR/macos-build-x64/latest-mac.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0-mac-x64.dmg
    sha512: fake-sha512-mac-x64
    size: 200000
EOF

# macOS arm64
touch "$ARTIFACTS_DIR/macos-build-arm64/ByteTensor-1.0.0-mac-arm64.dmg"
touch "$ARTIFACTS_DIR/macos-build-arm64/ByteTensor-1.0.0-mac-arm64.zip"
cat > "$ARTIFACTS_DIR/macos-build-arm64/latest-mac.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0-mac-arm64.dmg
    sha512: fake-sha512-mac-arm64
    size: 200000
EOF

# Linux x64
touch "$ARTIFACTS_DIR/linux-build-x64/ByteTensor-1.0.0.deb"
cat > "$ARTIFACTS_DIR/linux-build-x64/latest-linux.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0.deb
    sha512: fake-sha512-linux
    size: 300000
EOF

# Linux arm64
touch "$ARTIFACTS_DIR/linux-build-arm64/ByteTensor-1.0.0-arm64.deb"
cat > "$ARTIFACTS_DIR/linux-build-arm64/latest-linux-arm64.yml" <<'EOF'
version: 1.0.0
files:
  - url: ByteTensor-1.0.0-arm64.deb
...
for plat in "${WEB_PLATFORMS[@]}"; do
  ...
  mkdir -p "$dir"
  tarball="bytetensor-web-1.0.0-${plat}.tar.gz"
  touch "$dir/$tarball"
