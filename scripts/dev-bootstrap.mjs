...
const KILLABLE_NAMES = new Set(['electron', 'bytetensor', 'bytetensor.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','ByteTensor','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|ByteTensor|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'bytetensor'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
...
function launch(scriptName, withExtensions) {
  ...
  if (withExtensions) {
    env.BYTETENSOR_EXTENSIONS_PATH = path.resolve(process.cwd(), 'examples');
    log(`BYTETENSOR_EXTENSIONS_PATH=${env.BYTETENSOR_EXTENSIONS_PATH}`);
  }
