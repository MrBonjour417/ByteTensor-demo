#!/usr/bin/env bash
# ============================================================================
# ByteTensor — Ubuntu / Debian 一鍵自動化安裝腳本
# ============================================================================
# 功能：
...
#   5. 建立服務管理腳本 (/opt/ByteTensor/start-bytetensor.sh)
...
#   BYTETENSOR_MODE=desktop bash install-ubuntu.sh
# ============================================================================
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║          ByteTensor Installer for Ubuntu         ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
# ─── 取得版本號 ──────────────────────────────────────────────────────────────
resolve_version() {
    if [[ -n "${BYTETENSOR_VERSION:-}" ]]; then
        VERSION="$BYTETENSOR_VERSION"
        ...
        if [[ -z "$VERSION" ]]; then
            die "無法取得最新版本號，請手動指定: BYTETENSOR_VERSION=1.8.25 bash $0"
        fi
...
install_deb() {
    ...
    success "ByteTensor v${VERSION} 安裝完成"

    # 驗證安裝
    if command -v ByteTensor &>/dev/null || [[ -x /usr/bin/ByteTensor ]]; then
        success "ByteTensor 已安裝至 $(which ByteTensor 2>/dev/null || echo '/usr/bin/ByteTensor')"
    else
        warn "安裝可能不完整，找不到 ByteTensor 執行檔"
    fi
...
create_service_script() {
    local script_dir="/opt/ByteTensor"
    local script_path="${script_dir}/start-bytetensor.sh"
...
#!/bin/bash
# ============================================================================
# ByteTensor WebUI Headless 服務管理腳本
# 用法: ./start-bytetensor.sh [start|stop|restart|status|logs]
# ============================================================================

PIDFILE="/var/run/bytetensor.pid"
LOGFILE="/var/log/bytetensor.log"
WORKDIR="${BYTETENSOR_WORKDIR:-$HOME}"

start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "⚡ ByteTensor 已在執行中 (PID: $(cat "$PIDFILE"))"
        return 1
    fi

    echo "🚀 正在啟動 ByteTensor WebUI..."
    cd "$WORKDIR" || exit 1
    ...
    if kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ ByteTensor 啟動成功 (PID: $(cat "$PIDFILE"))"
        local ip
        ip=$(hostname -I 2>/dev/null | awk '{print $1}')
        echo "🌐 WebUI: http://${ip:-localhost}:25808"
    else
        echo "❌ ByteTensor 啟動失敗，請查看日誌: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ ByteTensor 執行中 (PID: $(cat "$PIDFILE"))"
...
case "${1:-}" in
    ...
    "")
        ...
        echo ""
        echo "環境變數:"
        echo "  BYTETENSOR_WORKDIR  - ByteTensor 工作目錄 (預設: \$HOME)"
        ;;
    *)
        echo "用法: $0 {start|stop|restart|status|logs}"
        exit 1
...
create_systemd_service() {
    ...
    local service_path="/etc/systemd/system/bytetensor.service"
...
ExecStart=/usr/bin/xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" /usr/bin/ByteTensor --webui --remote --no-sandbox
Restart=on-failure
RestartSec=10
...
SERVICE_EOF
    ...
    info "使用方式:"
    echo "    sudo systemctl start bytetensor     # 啟動"
    echo "    sudo systemctl stop bytetensor      # 停止"
    echo "    sudo systemctl enable bytetensor    # 開機自動啟動"
    echo "    sudo systemctl status bytetensor    # 查看狀態"
    echo "    journalctl -u bytetensor -f         # 查看日誌"
}

# ─── 建立桌面捷徑 ─────────────────────────────────────────────────────────────
create_desktop_entry() {
    local desktop_dir="${HOME}/.local/share/applications"
    local desktop_file="${desktop_dir}/bytetensor.desktop"
...
Exec=/usr/bin/ByteTensor --no-sandbox %U
Icon=ByteTensor
Terminal=false
Type=Application
Categories=Office;Utility;Development;
MimeType=x-scheme-handler/bytetensor;
StartupWMClass=ByteTensor
DESKTOP_EOF
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 ByteTensor v${VERSION} 安裝完成！${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 執行檔位置:${NC}  /usr/bin/ByteTensor"
    echo -e "  ${BOLD}📍 管理腳本:${NC}    /opt/ByteTensor/start-bytetensor.sh"
    echo ""
    ...
    if [[ "${MODE}" == "headless" ]]; then
        ...
        echo ""
        echo "    # 使用管理腳本"
        echo "    /opt/ByteTensor/start-bytetensor.sh start"
        echo "    /opt/ByteTensor/start-bytetensor.sh status"
        echo "    /opt/ByteTensor/start-bytetensor.sh stop"
        echo ""
        if command -v systemctl &>/dev/null; then
            echo "    # 或使用 systemd"
            echo "    sudo systemctl start bytetensor"
            echo "    sudo systemctl enable bytetensor  # 開機自啟"
            echo ""
        fi
        ...
        echo ""
        echo "    # 直接啟動（桌面環境）"
        echo "    ByteTensor --no-sandbox"
        echo ""
        echo "    # 或從應用程式選單尋找 ByteTensor"
        echo ""
    fi
    ...
    if [[ "${MODE}" == "headless" ]]; then
        echo -e "  ${YELLOW}💡 提示:${NC}"
        echo "     • 設定工作目錄: export BYTETENSOR_WORKDIR=/path/to/workspace"
