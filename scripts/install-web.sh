...
#   INSTALL_DIR=/opt/bytetensor-web bash install-web.sh
# ============================================================================
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     ByteTensor WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/bytetensor-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="bytetensor-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is bytetensor-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/bytetensor-web" ]]; then
        mv "${extract_temp}/bytetensor-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing bytetensor-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/bytetensor-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/bytetensor-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/bytetensor-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/bytetensor-web"
    local target_path="${INSTALL_DIR}/bytetensor-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by bytetensor-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 ByteTensor WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/bytetensor-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start ByteTensor WebUI"
        echo "    bytetensor-web start"
        echo ""
        echo "    # Check version"
        echo "    bytetensor-web version"
    else
        echo "    # Start ByteTensor WebUI (using full path)"
        echo "    ${INSTALL_DIR}/bytetensor-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/bytetensor-web ~/.local/bin/bytetensor-web"
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo ""
        echo "    # Remove symlink"
        echo "    rm ${BIN_DIR}/bytetensor-web"
    fi
