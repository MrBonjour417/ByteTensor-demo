...
const tarballName = `bytetensor-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
// 1. Prepare bundled-bytetensorcore
console.log('1. Preparing ByteTensorCore...');
...
const tarballContentDir = path.join(stagingDir, 'bytetensor-web');
...
const executableName = platform === 'win32' ? 'bytetensor-web.exe' : 'bytetensor-web';
...
// `bytetensor-web version` match the tarball filename.
...
// 7. Copy bundled-bytetensorcore
const backendSrc = path.join(projectRoot, 'resources/bundled-bytetensorcore', `${platform}-${arch}`);
const backendDest = path.join(tarballContentDir, 'bundled-bytetensorcore', `${platform}-${arch}`);
if (!fs.existsSync(backendSrc)) {
  throw new Error(`Backend bundle dir missing at ${backendSrc}. Ensure prepareByteTensorCore succeeded.`);
}
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} bytetensor-web`, {
