...
#   LOG_DIR            Directory for log files (default: ~/Library/Logs/ByteTensor)
...
LOG_DIR=${LOG_DIR:-$HOME/Library/Logs/ByteTensor}
...
cleanup_labels() {
  ...
  local stale_wts
  stale_wts=$(find /tmp -maxdepth 1 -name 'bytetensor-pr-*' -type d 2>/dev/null || true)
  if [ -n "$stale_wts" ]; then
