 ...
 *   BYTETENSOR_HUB_TAG    - Git tag to fetch from (default: 'dist-latest')
 *   BYTETENSOR_HUB_SKIP   - Set to '1' to skip hub resource preparation
 */
...
const BASE_URLS = [
  `https://raw.githubusercontent.com/iOfficeAI/AionHub/${process.env.BYTETENSOR_HUB_TAG || DEFAULT_TAG}/`,
  `https://cdn.jsdelivr.net/gh/iOfficeAI/AionHub@${process.env.BYTETENSOR_HUB_TAG || DEFAULT_TAG}/`,
];
...
async function prepareHubResources() {
  if (process.env.BYTETENSOR_HUB_SKIP === '1') {
    console.log('[hub] Skipping hub resource preparation (BYTETENSOR_HUB_SKIP=1)');
    return { skipped: true };
  }

  const tag = process.env.BYTETENSOR_HUB_TAG || DEFAULT_TAG;
