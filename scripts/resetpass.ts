 ...
 *   BYTETENSOR_DATA_DIR=/x bun run resetpass
 ...
 * See the comment there for why the default is `~/.bytetensor-web*` (not `~/.bytetensor*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `BYTETENSOR_PORT` to match
 ...
 * pass --port / BYTETENSOR_PORT if they launched webui on a non-default one.
 */
