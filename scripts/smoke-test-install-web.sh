...
export INSTALL_DIR="/tmp/bytetensor-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/bytetensor-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/bytetensor-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/bytetensor-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/bytetensor-web"
    exit 1
fi
...
VERSION_OUTPUT=$(bytetensor-web version 2>&1 || echo "")
