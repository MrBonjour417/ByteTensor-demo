...
if [ ! -d "$TEMP_DIR/bytetensor-web" ]; then
  echo "❌ Missing bytetensor-web directory"
  exit 1
fi

cd "$TEMP_DIR/bytetensor-web"

# New layout (bun compile standalone binary):
#   bytetensor-web/
#   ├── bytetensor-web           ← single compiled executable (no bin/, no dist/, no node_modules)
#   ├── package.json         ← for version lookup
#   ├── static/              ← SPA assets
#   └── bundled-bytetensorcore/<plat-arch>/...
for dir in static bundled-bytetensorcore; do
  if [ ! -d "$dir" ]; then
...
if [ ! -x "bytetensor-web" ]; then
  echo "❌ bytetensor-web is not executable"
  exit 1
fi
echo "✓ bytetensor-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./bytetensor-web version)
if [ -z "$VERSION" ]; then
...
BACKEND_DIR="bundled-bytetensorcore/$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/aarch64/arm64/; s/x86_64/x64/')"
BACKEND_BINARY="$BACKEND_DIR/bytetensorcore"
if [ ! -x "$BACKEND_BINARY" ]; then
  echo "❌ Backend binary missing or not executable: $BACKEND_BINARY"
  exit 1
fi
# ByteTensorCore has no --version flag. Read the pinned version from manifest.json
# (which the ByteTensorCore prepare script writes at pack time) and use --help to confirm the
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/bytetensor-web-data"
...
./bytetensor-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/bytetensor-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/bytetensor-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/bytetensor-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/bytetensor-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/bytetensor-web.html && grep -qE '<(div id="root"|script)' /tmp/bytetensor-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/bytetensor-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/bytetensor-web.html
  echo "---server log---"
  cat /tmp/bytetensor-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/bytetensor-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/bytetensor-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
