 ...
 *
 * Env vars:
 *   BYTETENSOR_PORT           : static server port (default 33000)
 *   BYTETENSOR_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   BYTETENSOR_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 ...
 *   $BYTETENSOR_DATA_DIR        env override (same effect)
 *   otherwise               ~/.bytetensor-web         (production)
 *                           ~/.bytetensor-web-dev     (dev, default)
 *                           ~/.bytetensor-web-dev-2   (dev + BYTETENSOR_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.bytetensor[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.bytetensor-dev` as a **symlink** to
 * `~/Library/Application Support/ByteTensor-Dev/bytetensor` so CLI tools (claude,
 ...
 * `.bytetensor-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.bytetensor-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.BYTETENSOR_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.BYTETENSOR_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.bytetensor-web${suffix}`);
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.BYTETENSOR_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.BYTETENSOR_ALLOW_REMOTE ?? process.env.BYTETENSOR_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.BYTETENSOR_STATIC_DIR) return process.env.BYTETENSOR_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(
    `Renderer assets not found at ${candidate}. Run "bun run package" first, or set BYTETENSOR_STATIC_DIR.`
  );
}
 ...
 *   $BYTETENSOR_NO_BUILD=1        : env-level opt-out
 *   $BYTETENSOR_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.BYTETENSOR_NO_BUILD)) return;
  if (process.env.BYTETENSOR_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set BYTETENSOR_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without ByteTensor.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.BYTETENSOR_LOG_DIR ?? path.join(workDir, 'logs');
