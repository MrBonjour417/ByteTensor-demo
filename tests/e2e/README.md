...
cd ../AionCore && cargo install --path crates/bytetensor-app
...
| -------------------------------- | ----- | ------------------------ |
| `BYTETENSOR_E2E_TEST`            | `1`   | App recognizes test mode |
| `BYTETENSOR_DISABLE_AUTO_UPDATE` | `1`   | No update checks         |
| `BYTETENSOR_DISABLE_DEVTOOLS`    | `1`   | No DevTools windows      |
| `BYTETENSOR_CDP_PORT`            | `0`   | CDP disabled             |
...
```bash
# Clean via database
sqlite3 "~/Library/Application Support/ByteTensor-Dev/bytetensor/bytetensor.db" \
