...
```typescript
return path.join(getDataPath(), 'bytetensor.db');
```

**userData directory** (`src/process/utils/configureChromium.ts:18-26`):

- Dev mode: `~/Library/Application Support/ByteTensor-Dev/` (macOS)
- Database: `{userData}/config/bytetensor.db`
- Shared by all E2E tests

**Conflict scenario**: If Assistant tests and Skills tests run in parallel workers:

1. Both access same `bytetensor.db` file
...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bytetensor-e2e-state-'));
...
```typescript
BYTETENSOR_EXTENSION_STATES_FILE: process.env.BYTETENSOR_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `bytetensor.db` → parallel execution impossible without refactoring.
...
1. Extend `BYTETENSOR_E2E_TEST` to include worker ID:
   ```typescript
   BYTETENSOR_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.BYTETENSOR_E2E_TEST_WORKER_ID || '0';
   return `ByteTensor-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/ByteTensor-E2E-Worker-0/config/bytetensor.db`
   - `~/Library/Application Support/ByteTensor-E2E-Worker-1/config/bytetensor.db`
...
**Complexity**: High (requires main process to read `BYTETENSOR_DATABASE_PATH`, conflicts with userData convention)
