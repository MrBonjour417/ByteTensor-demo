...
- ✅ SQLite 数据库独立(`bytetensor.db`)
- ✅ 扩展路径指向示例目录(`BYTETENSOR_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.bytetensor/skills/',     // My Skills 存储目录
...
- E2E 框架已设置 `BYTETENSOR_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bytetensor-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-bytetensor-skills`   |
...
- 共享同一个 SQLite 数据库(`bytetensor.db`)
- 并行会导致写锁冲突 + 测试数据污染
