 ...
 *      (e.g. out/linux-unpacked/bytetensor, out/mac-arm64/ByteTensor.app, out/win-unpacked/ByteTensor.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/ByteTensor.exe  or  out/win-x64-unpacked/ByteTensor.exe
    ...
    // out/mac-arm64/ByteTensor.app/Contents/MacOS/ByteTensor  or  out/mac/ByteTensor.app/...
    ...
    // Linux: out/linux-unpacked/bytetensor  (lowercase executable name)
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    BYTETENSOR_EXTENSIONS_PATH: process.env.BYTETENSOR_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
