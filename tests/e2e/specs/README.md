...
## 一、先理解 ByteTensor Team 是什么

**ByteTensor Team 是一个由 leader 驱动的 AI 团队系统。用户不直接操作团队成员，用户只跟 leader 说话。**
...
**禁止用于触发任何操作**——添加成员、解雇成员、发送消息等，必须且只能通过 leader 聊天输入框。如果你用 invokeBridge 触发了操作，你测的不是 ByteTensor Team，你测的是一个普通的 RPC 接口。
