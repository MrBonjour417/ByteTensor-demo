...
test.describe('Agent Settings Detection', () => {
  ...
  test('detected CLI agents displayed', async ({ page }) => {
    ...
    const hasKnownAgent = ['Claude', 'Codex', 'Gemini', 'ByteTensor', 'OpenCode', 'Qwen'].some((name) =>
      body?.includes(name)
    );
  ...
  test('Gemini agent is present in detected list', async ({ page }) => {
    ...
    // Gemini or ByteTensor CLI should be in the agent list
    await expectBodyContainsAny(page, ['Gemini', 'gemini', 'ByteTensor']);
  });
