...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bytetensor-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bytetensor-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      BYTETENSOR_EXTENSIONS_PATH: emptyExtensionsDir,
      BYTETENSOR_EXTENSION_STATES_FILE: extensionStatesFile,
