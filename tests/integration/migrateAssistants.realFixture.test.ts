 ...
 * (`/Users/zhoukai/Downloads/bytetensor-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `bytetensor-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/bytetensor-config.txt';
...
function resolveBackendBinary(): string {
  ...
  throw new Error('aioncore binary not found (set BYTETENSOR_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `bytetensor-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'bytetensor-migrate-fixture-'));
