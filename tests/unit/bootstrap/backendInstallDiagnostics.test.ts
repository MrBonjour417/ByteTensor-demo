...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ['C:\\ByteTensor\\resources', { mtimeMs: 1000, size: 0 }],
      ['C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        resourcesPath: 'C:\\ByteTensor\\resources',
        checkedBundledPath: 'C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64\\bytetensorcore.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
        execPath: 'C:\\ByteTensor\\ByteTensor.exe',
        isPackaged: true,
        platform: 'win32',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'bytetensorcore.exe',
      binaryPath: 'C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64\\bytetensorcore.exe',
      bundledDirPath: 'C:\\ByteTensor\\resources\\bundled-bytetensorcore',
      execPath: 'C:\\ByteTensor\\ByteTensor.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['bytetensorcore.exe'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\ByteTensor\\resources\\bundled-bytetensorcore\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});
