...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ...
      ['C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        checkedBundledPath: 'C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64\\aioncore.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'aioncore.exe',
      binaryPath: 'C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64\\aioncore.exe',
      bundledDirPath: 'C:\\ByteTensor\\resources\\bundled-aioncore',
      execPath: 'C:\\ByteTensor\\ByteTensor.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['aioncore.exe'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\ByteTensor\\resources\\bundled-aioncore\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});
