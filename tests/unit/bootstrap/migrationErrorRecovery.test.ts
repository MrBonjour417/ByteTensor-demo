...
describe('migrationErrorRecovery', () => {
  ...
  it('closes driver when initSchema throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/bytetensor.db')).rejects.toThrow('Schema init failed');
  ...
  it('closes driver when runMigrations throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/bytetensor.db')).rejects.toThrow('Migration step failed');
    expect(mockDriver.close).toHaveBeenCalled();
  });
  ...
  it('closes driver when setDatabaseVersion throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/bytetensor.db')).rejects.toThrow('Set version failed');
    expect(mockDriver.close).toHaveBeenCalled();
  });
  ...
  it('closes driver when system user insert throws', async () => {
    ...
    await expect(runLegacyDatabaseMigrations('/test/bytetensor.db')).rejects.toThrow('User insert failed');
    expect(mockDriver.close).toHaveBeenCalled();
  });
