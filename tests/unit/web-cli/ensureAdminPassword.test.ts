...
describe('ensureAdminPassword', () => {
  it('seeds password on fresh install (needs_setup=true)', async () => {
    ...
    expect(logs).toContain('[bytetensor-web] Generated initial admin password: SuperSecret123');
  ...
  it('accepts top-level new_password in reset-password response', async () => {
    ...
    expect(logs).toContain('[bytetensor-web] Generated initial admin password: FromTopLevel');
  });
  ...
  it('uses caller-supplied resetCommand in the "Forgot the password" hint', async () => {
    ...
    expect(logs.every((m) => !m.includes('bytetensor-web resetpass'))).toBe(true);
  });

  it('defaults to `bytetensor-web resetpass` when resetCommand is not provided', async () => {
    const { deps, logs } = makeDeps({
      handlers: [
        () => mockResponse(200, { needs_setup: false }),
        () => mockResponse(200, { data: { username: 'admin' } }),
    ...
    expect(logs.some((m) => m.includes('bytetensor-web resetpass'))).toBe(true);
  });
